...
function openHealthQuestion() {
    $("#healthidContent").delay(300).show(200);
    $("#healthid1").hide();
    $("#healthid").show();
}
function openSubHealthQuestion() {
    $("#id1").show();
    // $("#healthid").show();
    // $("#healthidContent .doesdont_sectionEssure1").toggleClass("doesdont_sectionEssure2");
}
function fadeOut() {
    $("#medical").hide();
    $("#medical1").show();
    $("#medicalContent").delay(300).show(200);
}
...
$(document).ready(function() {
    ...
    $("#medical1").click(function() {
        $("#medicalContent").delay(300).hide(200);
        $("#medical1").hide();
        $("#medical").show();
    });
...
jQuery(function() {
    ...
    jQuery("#ValidPinCode").validate({
        expression: "if (VAL.match(/^[0-9]*$/) && VAL) { if(VAL.length==6){  getCityName(); return true; } else { jQuery('#errordisplayPrashant').show(); return false; } } else { jQuery('#errordisplayPrashant').show(); return false;}",
        message: "Please enter pin code"
    });
    jQuery("#NomineeName").validate({
        expression: "if (VAL !='')  return true; else  return false;"

    });
    jQuery("#nomineeRelation").validate({
        expression: "if (VAL != '0'){  $('#ValidRelationAddClass').removeClass();$('#ValidRelationAddClass').addClass('drop_Health'); return true; }else{ $('#ValidRelationAddClass').addClass('ErrorField'); return false;}"
    });
    for (k = 1; k <= t; k++) {
        ...
        jQuery("#occupation-" + k).validate({
            expression: "if (VAL.match(/^[a-zA-Z\(\)]+$/) && VAL) { return true; } else { jQuery('#errordisplayPrashant').show(); return false; }",
            message: "Please enter valid occupation"
    ...
    jQuery("#medical_question1").validate({
        expression: "if (isChecked(SelfID)){ return true; } else { openMedicalQues(); return false; } "

    });
    jQuery("#medical_question2").validate({
        expression: "if (isChecked(SelfID)){ return true; } else { openMedicalQues(); return false; } "

    });
    jQuery("#medical_question3").validate({
        expression: "if (isChecked(SelfID)){ return true; } else { openMedicalQues(); return false; } "

    });
    jQuery("#medical_question4").validate({
        expression: "if (isChecked(SelfID)){ return true; } else { openMedicalQues(); return false; } "
    });

    var health_ques = 10;
    for (var p = 1; p <= health_ques; p++) {

        jQuery("#health_question" + p).validate({
            expression: "if (isChecked(SelfID)){ return true; } else { openHealthQues(); return false; } "
        });
    }

    var health_sub_ques = 11;
    for (p = 1; p <= health_sub_ques; p++) {

        jQuery("#health_sub_question" + p).validate({
            expression: "if (isChecked(SelfID)){ return true; } else { openHealthSubQues(); return false; } "
        });
    }
    ...
    jQuery('.AdvancedForm').validated(function() {
        ...
        document.savePagesecure5.submit();
...
function openHealthSubQues() {
    $("#suffer").delay(300).show(200);
    $("#id1").toggle();
    $("#healthidContent .doesdont_sectionEssure1").toggleClass("doesdont_sectionEssure2");
}
