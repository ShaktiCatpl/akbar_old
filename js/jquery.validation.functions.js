...
function changeTitleList() {
    ...
    if (n != '0') {
        ...
        $("#ValidSubTitleAddClass-" + n).removeClass();
        $("#ValidSubTitleAddClass-" + n).addClass('drop_Health_title');
        $("#titleCd-" + n).next('.ValidationErrors').remove();
...
jQuery(function() {
    ...
    jQuery("#ValidNomniee").validate({
        expression: "if (VAL.match(/^[a-zA-Z\(\)]+$/) && VAL) { return true; } else { jQuery('#errordisplayPrashant').show(); return false; }",
        message: "Please enter nomniee name."
    });
    ...
    for (k = 1; k <= t; k++) {
        jQuery("#relationCd-" + k).validate({
            expression: "if (VAL != '0'){  $('#ValidRelationAddClass-" + k + "').removeClass();$('#ValidRelationAddClass-" + k + "').addClass('drop_Health11'); return true; }else{ $('#ValidRelationAddClass-" + k + "').addClass('ErrorField'); return false;}"
        });
        ...
        jQuery("#lastNamecd-" + k).validate({
         ...
         jQuery("#passport-" + k).validate({
            expression: "if (VAL.match(/^[-a-zA-Z0-9_ ]+$/) && VAL) { return true; } else { jQuery('#errordisplayPrashant').show(); return false; }",
