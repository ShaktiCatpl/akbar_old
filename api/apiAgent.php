...
function getXMLClaimResponse($data,$tagTitle,$fileName) {	
	...
	xml_parse_into_struct($xmlparser,$xmlResponse1,$sum);
	xml_parser_free($xmlparser);
	if(isset($sum[21]['value'])) {
		if($sum[21]['value']=='QueryException:Query failed') {
			return 'Error:QueryException:Query failed';
		}	else	{
			return 'Amount:'.$sum[21]['value'];
		}
	}  
		elseif(isset($sum[22]['value'])){
		return 'Error:'.$sum[22]['value'];
	}	else {
		return 'Error:oh! something went wrong.';
		return false;
	}    
		//$result1 =  new  xml2array($result->root_element->myDOMNode->textContent);
		//return false;
