...
if (isset($_POST)) {
	...
	"QuotationDOB" => sanitize_data($_POST['QuotationDOB']),
    );
...
//    if ($baseProductId == 11001001) {
//        $siResult = $sunInsuredEnhganceOneArray[$sumInsured];
//        $proposalDummySi = $siResult['sumInsured'];
//        $addonsData = "EXPERTOPNENH1";
//    } else if ($baseProductId == 11001002) {
//        $siResult = $sunInsuredEnhganceTwoArray[$sumInsured];
//        $proposalDummySi = $siResult['sumInsured'];
//        $addonsData = "EXPERTOPNENH2";
//    }
    $proposalDummySi = sanitize_data($_POST['proposalDummySi']);
	if($proposalDummySi=='50000') {
		$sumInsured = "002";
	}	else {
		$sumInsured = "001";
	}
    $baseProductId = "10003004";	//10001101
    $qnaT = $_POST['questions'];
    $qna = base64_encode(json_encode($qnaT));
    $businessTypeCd = "NEWBUSINESS";
    $baseAgentId = sanitize_data($_POST["agentId"]);
   // $baseAgentId = "20008802";	//20008025
   ...
   // $guid = 41;
    ...
    if ($checkclickid == 1) {
        ...
        file_put_contents("data/care/" . $fileNameRequest . "_createPolicyCare_Request.xml", $xmlData);
        $options = array(
            'trace' => true,
            'exceptions' => 1,
            'style' => SOAP_DOCUMENT,
        ...
        $action = "urn:createPolicy";
        $res = $client->__doRequest($xmlData, $location, $action, SOAP_1_2);
        $xml = new xml2array($res);
        $dataArr = $xml->getResult();
        file_put_contents("data/care/" . $fileNameRequest . "_createPolicyCare_Response.xml", $res);
        $getPropasalNumber = sanitize_data($dataArr['soapenv:Envelope']['soapenv:Body']['ns:createPolicyResponse']['ns:return']['int-policy-data-iO']['policy']['proposal-num']['#text']);
        $getPropasalPrimiumNumber = sanitize_data($dataArr['soapenv:Envelope']['soapenv:Body']['ns:createPolicyResponse']['ns:return']['int-policy-data-iO']['policy']['premium']['#text']);
         $errorData = $dataArr['soapenv:Envelope']['soapenv:Body']['ns:createPolicyResponse']['ns:return']['int-policy-data-iO']['error-lists']['err-description']['#text'];
        if ($errorData != '') {
   ...
   <soap:Body>
      <rel:UserConfirmation>
         ...
         <rel:intIO>
            ...
            file_put_contents("data/care/" . $fileNameRequest . "_User_Confirmation_Request.xml", $checkUserCreation);

            $client = new SoapClient(WSDLURL, $options);
            $location = ENDPOINTURL;
            $action = "urn:userConfirmation";
            $res = $client->__doRequest($checkUserCreation, $location, $action, SOAP_1_2);
            file_put_contents("data/care/" . $fileNameRequest . "_User_Confirmation_Respose.xml", $res);
            $xml = new xml2array($res);
            $dataArr = $xml->getResult();
   ...
   <soap:Body>
      <rel:userCreation>
         ...
         <rel:intIO>
            ...
            <xsd:userId></xsd:userId>
                ...
                file_put_contents("data/care/" . $fileNameRequest . "_User_Creation_Request.xml", $userCreation);
                $action1 = "urn:userCreation";
                $res1 = $client->__doRequest($userCreation, $location, $action1, SOAP_1_2);
                file_put_contents("data/care/" . $fileNameRequest . "_User_Creation_Response.xml", $res1);
    ...
    USER_STATUS
          ...
          $query['leadstage'] = sanitize_data("Semi filled Proposal");
          $query['subject'] = sanitize_data("Semi filled Proposal");
          $query['rhiproduct'] = sanitize_data("Care");
          ...
          $subject = 'Care Proposal Page';
          saveContinueMail($titleCd,$firstName,$lastName,$saveandcontinueemail, $subject, $proposalTenourCode, $totalChildMember, $totalMember, $proposalageGroupOfEldestMember, $PREMIUM_AMOUNT, $proposalDummySi, $policyId,'CARE',$query);
    }
} else {
